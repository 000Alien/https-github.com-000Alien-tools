            ...
            if fund_code_input:
                with st.spinner("获取基金持仓中..."):
                    ...
                    if not holdings_df.empty:
                        ...
                        create_download_buttons(holdings_df, f"基金_{fund_code}_持仓", "single")
                    else:
                        st.error("未获取到持仓数据")
            else:
                st.warning("请输入基金代码")
    ...
    if all_hold:
        ...
        if fund_holdings_dict:
            ...
            if selected_fund:
                ...
                create_download_buttons(fund_df, f"基金_{selected_fund}_持仓", "single_select")
...
with col_clear1:
    if st.button("🗑️ 清空高涨幅基金数据", type="secondary", use_container_width=True):
        st.session_state.high_funds = pd.DataFrame()
        st.success("已清空高涨幅基金数据")

with col_clear2:
    if st.button("🗑️ 清空持仓明细数据", type="secondary", use_container_width=True):
        st.session_state.all_holdings = pd.DataFrame()
        st.session_state.fund_holdings_dict = {}
        st.success("已清空持仓明细数据")

with col_clear3:
    if st.button("🗑️ 清空龙头评分数据", type="secondary", use_container_width=True):
        st.session_state.stock_scores = pd.DataFrame()
        st.success("已清空龙头评分数据")
...
# 手动导入列表（放在侧边栏或下方）
st.divider()
with st.expander("📋 手动导入基金列表（高级功能）", expanded=False):
    uploaded_file = st.file_uploader("上传 Excel / CSV（必须包含 '基金代码' 列）", type=['xlsx', 'xls', 'csv'])

    st.markdown("**或直接粘贴基金代码（每行一个）**")
    manual_input = st.text_area("", height=100, placeholder="022364\n022365\n001234 易方达某某基金")

    if st.button("🚀 从导入列表获取持仓", type="primary"):
...
if not st.session_state.high_funds.empty:
    with st.expander("📈 高涨幅基金列表", expanded=False):
        ...
        st.markdown("**💾 下载**")
        create_download_buttons(st.session_state.high_funds, "高涨幅基金列表", "result_high")
...
if not st.session_state.all_holdings.empty:
    with st.expander("📥 基金持仓明细", expanded=False):
        ...
        st.markdown("**💾 下载**")
        create_download_buttons(st.session_state.all_holdings, "基金持仓明细", "result_holdings")
...
if not st.session_state.stock_scores.empty:
    with st.expander("🐉 龙头股评分排行", expanded=True):
        ...
        st.markdown("**💾 下载**")
        create_download_buttons(st.session_state.stock_scores[cols].head(TOP_N_STOCKS), "龙头股排行", "result_score")

st.caption("✅ 已完全修复JSON解析错误 | 支持CSV/Excel下载 | 支持单只基金下载 | 高涨幅基金支持下载")