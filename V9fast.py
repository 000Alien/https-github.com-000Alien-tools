...
RETURN_PERIOD = st.sidebar.selectbox("基金涨幅周期", ["月", "季度", "年", "自定义"], index=2)
...
USE_AKSHARE = st.sidebar.checkbox("启用 AKShare 兜底/单只优先", value=True)
HOLDING_WORKERS = st.sidebar.slider("持仓并发数", 1, 16, 8)
HOLDING_DELAY = st.sidebar.slider("持仓请求抖动(秒)", 0.0, 2.0, 0.2, 0.1)
FAST_DIRECT_MODE = st.sidebar.checkbox("批量优先直连东方财富", value=True)
