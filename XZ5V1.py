# -*- coding: utf-8 -*-
"""
股票分析工具 v3.1 - 多线程 + 断点续传 + 增强实时行情 + 所属行业
功能：
1. 个股分析：历史价格 + 资金流 + 龙虎榜 + 实时报价 + 主力大额流入筛选
2. 龙虎榜扫描：全市场扫描 + 汇总统计 + 增强实时行情（最新价/涨跌幅/换手率/量比/振幅）+ 所属行业
优化点：
- 8线程并行处理个股
- 支持断点续传（个股历史价格只拉取新增部分）
- 个股导出Excel增加「实时与汇总」Sheet + 「主力大额流入日 (>5000万)」Sheet
- 龙虎榜结果增加多维度实时行情 + 所属行业
支持中途停止任务
"""
...
class StockAnalyzerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("股票分析工具 v3.1 - 多线程 + 增强实时行情 + 所属行业")
        ...
        result_frame = ttk.LabelFrame(self.tab_scan, text="扫描结果（含实时行情 + 所属行业）", padding=5)
    ...
    def _worker(self, start_date, end_date, save_dir):
        try:
            if self.current_mode == "single":
                ...
                self.task_queue.put(("log", f"启动 8 线程并行处理 {len(codes)} 只股票（支持断点续传）...", "single"))
                ...
                with ThreadPoolExecutor(max_workers=8) as executor:
                    ...
                    for code in codes:
                        ...
                        futures.append(executor.submit(
                            ...
                            code, start_date, end_date, lhb_all, save_dir
                        ))
        ...
        try:
            ...
            if os.path.exists(excel_path):
                try:
                    with pd.ExcelFile(excel_path) as xls:
                        if '历史价格' in xls.sheet_names:
                            existing_price = pd.read_excel(xls, '历史价格')
                            if '日期' in existing_price.columns:
                                existing_price['日期'] = pd.to_datetime(existing_price['日期'])
                                existing_max_date = existing_price['日期'].max()
                                self.task_queue.put(("log", f"  已有历史文件 → 最后日期 {existing_max_date.date()}", "single"))
                except Exception as e:
                    self.task_queue.put(("log", f"  读取历史文件失败（将全量更新）: {e}", "single"))

            # 决定本次拉取的起始日期
            fetch_start = start
            if existing_max_date is not None:
                next_day = (existing_max_date + pd.Timedelta(days=1)).strftime("%Y%m%d")
                if next_day > start:
                    fetch_start = next_day
                    self.task_queue.put(("log", f"  增量拉取 {fetch_start} ~ {end}", "single"))

            # 获取新增价格数据
            price_df_new = pd.DataFrame()
            if fetch_start <= end:
                try:
                    price_df_new = ak.stock_zh_a_hist(
                        symbol=code, period="daily", start_date=fetch_start,
                        end_date=end, adjust="qfq"
                    )
                    if not price_df_new.empty:
                        price_df_new['日期'] = pd.to_datetime(price_df_new['日期'])
                        self.task_queue.put(("log", f"  新增价格数据：{len(price_df_new)} 条", "single"))
                except Exception as e:
                    self.task_queue.put(("log", f"  价格拉取失败: {e}", "single"))

            # 合并历史 + 新增数据
            if not existing_price.empty and not price_df_new.empty:
                price_df = pd.concat([existing_price, price_df_new], ignore_index=True)
                price_df = price_df.drop_duplicates(subset=['日期']).sort_values('日期').reset_index(drop=True)
            elif not price_df_new.empty:
                price_df = price_df_new
            else:
                price_df = existing_price

            if price_df.empty:
                self.task_queue.put(("log", "  无可用价格数据", "single"))
            ...
            with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
                ...
                summary_data = {
                    "股票代码": [code],
                    "股票名称": [stock_name],
                    "实时最新价": [realtime_price if realtime_price is not None else "N/A"],
                    "分析日期范围": [f"{start} ~ {end}"],
                    "最后更新时间": [datetime.now().strftime("%Y-%m-%d %H:%M:%S")]
                }
                pd.DataFrame(summary_data).to_excel(writer, "实时与汇总", index=False)

                # 新增：主力净流入 > 5000万 的日期
                if not fund_df.empty and '主力净额' in fund_df.columns:
                    fund_numeric = pd.to_numeric(fund_df['主力净额'], errors='coerce')
                    big_inflow = fund_df[fund_numeric > 50000000].copy()
                    if not big_inflow.empty:
                        big_inflow.to_excel(writer, "主力大额流入日 (>5000万)", index=False)
                        self.task_queue.put(("log", f"  ★ 发现 {len(big_inflow)} 天主力净流入 > 5000万元", "single"))
    ...
    def _process_lhb_scan(self, start, end, save_dir):
        ...
        # 获取增强实时行情
        self.log("正在获取全市场实时行情（最新价 + 涨跌幅 + 换手率 + 量比 + 振幅）...", "scan")
        try:
            realtime_df = ak.stock_zh_a_spot_em()
            realtime_df = realtime_df[['代码', '最新价', '涨跌幅', '换手率', '量比', '振幅']].copy()

            for col in ['最新价', '涨跌幅', '换手率', '量比', '振幅']:
                realtime_df[col] = pd.to_numeric(realtime_df[col], errors='coerce')

            realtime_df['涨跌幅'] = realtime_df['涨跌幅'].round(2)
            realtime_df['换手率'] = realtime_df['换手率'].round(2)
            realtime_df['量比']   = realtime_df['量比'].round(2)
            realtime_df['振幅']   = realtime_df['振幅'].round(2)
            ...
            summary.rename(columns={'代码_x': code_col}, inplace=True)
            ...
            self.log(f"成功匹配 {len(realtime_df)} 只股票的增强实时行情", "scan")

        except Exception as e:
            self.log(f"增强实时行情获取失败: {str(e)[:120]}，回退到仅最新价模式", "scan")
            spot_dict = self._get_all_realtime_spot()
            summary['最新价'] = summary[code_col].map(spot_dict).fillna("N/A")
            for col in ['涨跌幅', '换手率', '量比', '振幅']:
                summary[col] = "N/A"

        # 获取所属行业
        self.log("正在获取各股票所属行业信息（约需几秒）...", "scan")
        industry_dict = {}
        codes_list = summary[code_col].tolist()
            ...
            try:
                ...
                if not info.empty:
                    ...
                    if not industry_row.empty:
                        industry_dict[code] = industry_row['value'].iloc[0]
                    else:
                        industry_dict[code] = "未知"
                else:
                    industry_dict[code] = "未知"
            except:
                industry_dict[code] = "获取失败"
            time.sleep(0.08)  # 防限频

        summary['所属行业'] = summary[code_col].map(industry_dict).fillna("N/A")
