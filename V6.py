...
st.markdown("**持仓解析最终版 V5**：支持自定义爬取周期 | 优化布局")
...
def crawl_high_return_funds():
    ...
    while page <= MAX_PAGES:
        ...
        try:
                    ...
                    try:
                        ...
                        if ret >= RETURN_THRESHOLD:
                            funds.append({
                                '基金代码': fields[0],
                                '基金名称': fields[1],
                                f'{period_type}涨幅(%)' if period_type != "自定义" else f'{start_date}至{end_date}涨幅(%)': round(
                                    ret, 2)
                            })
                    except:
                        continue
            ...
            st.warning(f"第 {page} 页请求失败: {str(e)[:50]}")
            page += 1
            continue
...
st.caption("✅ V5 增强版 | 支持自定义爬取周期（近1月/3月/6月/1年/2年/3年/自定义日期）| 下载按钮跟随数据")
